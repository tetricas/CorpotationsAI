    ...
    m_isConnected(false)
{
    ...
    connect( &m_map, &CClient::connected, [this](){m_isConnected = true; emit isConnectedChanged();});
    connect( &m_map, &CClient::notConnected, [this](){m_isConnected = false; emit isConnectedChanged();});
    //connect( &m_map, &CClient::updateData, this, &CGameSpace::update);
...
void CGameSpace::reconnect()
{
    m_map.setConnection();
}

void CGameSpace::makeServer()
{
...
void CGameSpace::paint(QPainter* painter)
{
    for( int i(0); i < m_blocksCount; ++i)
        for (int j(0); j < m_blocksCount; ++j)
        {
            ...
            QRect rect(i*m_blockSideSize, j*m_blockSideSize,
                       m_blockSideSize, m_blockSideSize);
            painter->fillRect(rect, QBrush(QColor(m_map.getBlock(i,j).second)));
        }
    QPen pen(Qt::blue, 2);
    painter->setPen(pen);
    painter->drawRect(m_iPosX*m_blockSideSize, m_iPosY*m_blockSideSize,
                      m_blockSideSize, m_blockSideSize);
}
